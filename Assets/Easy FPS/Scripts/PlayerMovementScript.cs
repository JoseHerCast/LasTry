...
public class PlayerMovementScript : MonoBehaviour {
	...
	void Awake(){
		...
		lintern = transform.Find("Spot Light").transform.gameObject.GetComponent<Light>();
	...
	*/
	void WalkingSound(){
		if (_walkSound && _runSound) {
			if (grounded) { //for walk sounds using this because suraface is not straigh			
				if (currentSpeed > 1) {
					//print ("Personaje en movimiento");
					if (maxSpeed == 3) {
						//print ("Caminando");
						if (!_walkSound.isPlaying) {
							//print ("Reproduciendo sonido de caminar");
							_walkSound.Play ();
							_runSound.Stop ();
						}					
					} else if (maxSpeed == 5) {
						//print ("Reproduciendo sonido de correr");
					...
					//print("Sonidos detenidos por que no se mueve el personaje");
					_walkSound.Stop ();
					_runSound.Stop ();
				}
            } else {
				//print("Sonidos detenidos por que no esta aterrizado");
            _walkSound.Stop();
            _runSound.Stop();
        }
    } else {
