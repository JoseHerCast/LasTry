...
public class GunScript : MonoBehaviour {
	...
	void Awake(){
		...
		{
			if (Bullets.bullets.semiBulletsIHave > 0)
			{
				bulletsIHave = Bullets.bullets.semiBulletsIHave;
				bulletsInTheGun = Bullets.bullets.semiBulletsInWeapon;
			}
		}
		if (currentStyle == GunStyles.automatic)
		{
			if (Bullets.bullets.autoBulletsIHave > 0)
			{
				bulletsIHave = Bullets.bullets.autoBulletsIHave;
				bulletsInTheGun = Bullets.bullets.autoBulletsInWeapon;
			}
		}
	...
	void Shooting(){
		...
		if (!meeleAttack) {
			if (currentStyle == GunStyles.nonautomatic) {
				...
				Bullets.bullets.semiBulletsIHave = bulletsIHave;
				Bullets.bullets.semiBulletsInWeapon = bulletsInTheGun;
			}
			if (currentStyle == GunStyles.automatic) {
				if (Input.GetButton ("Fire1")) {
					ShootMethod ();
				}
				Bullets.bullets.autoBulletsIHave = bulletsIHave;
				Bullets.bullets.autoBulletsInWeapon = bulletsInTheGun;
			}
		}
	...
	private void ShootMethod(){
		if(waitTillNextFire <= 0 && !reloading && pmS.maxSpeed < 5){
			...
			if(bulletsInTheGun > 0){
				...
				//With this we can get the target and its properties and methods when a ray hit it, for destroying it
				RaycastHit hit;
				if (Physics.Raycast(secondCamera.transform.position, secondCamera.transform.forward, out hit, range))
				{
